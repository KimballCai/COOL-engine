...
public class QueryServerModel {
  ...
  public static ResponseEntity<List<String>> cohortAnalysis(ExtendedCohortQuery query) {
    ...
    try {
      ...
      if (!query.isValid()) throw new IOException("[x] Invalid cohort query.");
      ...
      List<ExtendedResultTuple> results =
  ...
  public static ResponseEntity<String> precessIcebergQuery(IcebergQuery query) {
    try {
      ...
      List<BaseResult> results =
          coolModel.olapEngine.performOlapQuery(coolModel.getCube(inputSource), query);
