...
public class CoolCohortEngine {
  ...
  public List<Integer> selectCohortUsers(CubeRS cube, InputVector users, ExtendedCohortQuery query)
      throws IOException {
    if (cube == null) throw new IOException("data source is null");
  ...
  public void createCohort(ExtendedCohortQuery query, List<Integer> users, File cohortRoot)
    ...
    for (int v : userArray) if (v > maxValue) maxValue = v;

    Compressor compressor =
        new ZIntBitCompressor(
            Histogram.builder()
                .max(maxValue)
    ...
    int nbytes =
        compressor.compress(userArray, 0, userArray.length, compressed, 0, compressed.length);
    stream.write(compressed, 0, nbytes);
    stream.writeBytes(query.toString());
    stream.close();
  }

  public List<ExtendedResultTuple> processCohortResult(
    ...
    for (Map.Entry<ExtendedCohort, Map<Integer, List<Double>>> entry : resultMap.entrySet()) {
      ...
      for (Map.Entry<Integer, List<Double>> ageMetric : entry.getValue().entrySet()) {
        ...
        if (!sigma.getAgeFieldFilter().accept(age)) continue;
   ...
   * @return result
   */
  public List<ExtendedResultTuple> performCohortQuery(
      CubeRS cube, InputVector users, ExtendedCohortQuery query) {
    List<CubletRS> cublets = cube.getCublets();
