...
public class IcebergSelection {
    ...
    if (filter.getType().equals(SelectionQuery.SelectionType.filter)) {
      FieldFilter fieldFilter =
          FieldFilterFactory.create(
              this.tableSchema.getFieldSchema(selection.getDimension()),
              null,
              selection.getValues());
   ...
   * @return map of < time_range : bitMap >, where each element in biteMap is one record, true = the
   *     record meet the requirement, false = the record don't meet the requirement.
   */
        ...
        for (int i = 0; i < timeInput.size(); i++) {
          ...
          if (time >= min && time <= max) {
            ...
            while (!(time >= this.minTimeRanges.get(timeRangeKey)
                && (time <= this.maxTimeRanges.get(timeRangeKey)))) timeRangeKey += 1;
