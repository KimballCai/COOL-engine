...
public class SetFilter implements Filter {
  ...
  public static Filter GenerateSetFilter(
      String fieldSchema, String[] acceptValues, String[] rejectValues) {
    if (acceptValues != null) return new SetAcceptFilter(fieldSchema, acceptValues);
    else if (rejectValues != null) return new SetRejectFilter(fieldSchema, rejectValues);
    else
