...
public class FastInputStream extends InputStream {
  ...
  /**
   * An array of bytes that was provided by the creator of the stream. Elements <code>buffer[0]
   * </code> through <code>buffer[count-1]</code> are the only bytes that can ever be read from the
   ...
   * from the input stream buffer will be <code>buffer[pos]</code>.
   */
  protected int pos;
   ...
   *
   * <p>If no mark has been set, then the value of mark is the offset passed to the constructor (or
   ...
   * and the initial value of <code>count</code> is the length of <code>buffer</code>.
   *
   ...
   * The initial value of <code>pos</code> is <code>offset</code> and the initial value of <code>
   * count</code> is the minimum of <code>offset+length</code> and <code>buffer.length</code>. The
   * buffer array is not copied. The buffer's mark is set to the specified offset.
   *
   * @param buffer the input buffer.
   ...
   * file. Otherwise, the number <code>k</code> of bytes read is equal to the smaller of <code>len
   * </code> and <code>count-pos</code>. If <code>k</code> is positive, then bytes <code>buffer[pos]
   * </code> through <code>buffer[pos+k-1]</code> are copied into <code>b[off]</code> through <code>
   * b[off+k-1]</code> in the manner performed by <code>System.arraycopy</code>. The value <code>k
   * </code> is added into <code>pos</code> and <code>k</code> is returned.
   *
   * <p>This <code>read</code> method cannot block.
   *
   ...
   * stream.
   *
   * <p>The value returned is <code>count&nbsp;- pos</code>, which is the number of bytes remaining
   ...
   *
   * <p>If no mark has been set, then the value of the mark is the offset passed to the constructor
