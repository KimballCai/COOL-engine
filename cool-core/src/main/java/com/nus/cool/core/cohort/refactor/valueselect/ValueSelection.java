...
public class ValueSelection {
  ...
  @Getter private List<Filter> filterList;

  @JsonIgnore private List<String> schemaList;
  // private

  @JsonIgnore @Getter private AggregateFunc aggregateFunc;
  ...
  public boolean IsSelected(ProjectedTuple tuple) {
    for (Filter filter : filterList) {
      if (!filter.accept((Integer) tuple.getValueBySchema(filter.getFilterSchema()))) return false;
    }
    return true;
  }
}
