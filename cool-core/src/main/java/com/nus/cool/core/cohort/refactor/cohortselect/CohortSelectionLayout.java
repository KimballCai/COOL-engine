...
/**
 * Mappered from json file into CohortSelectionLayout Parse it to generate different CohortSelection
 * instance.
 */
@Getter
public class CohortSelectionLayout {
  ...
  private CohortRangeSelector generateRangeSelector() {
    Preconditions.checkArgument(
        this.max != null, "Max attribute in RangeCohortSelection should not be NULL");
    Preconditions.checkArgument(
        this.min != null, "Min attribute in SetCohortSelection should not be NULL");
  ...
  private CohortSetSelector generateSetSelector() {
    /**
     * If set cohort selector, and no rejectValue and acceptValue input. We consider the situation
     * that all value in this schema can be chosen as a
     */
