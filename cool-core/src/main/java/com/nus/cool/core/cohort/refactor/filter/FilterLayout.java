...
public class FilterLayout {
  @Getter private String fieldSchema;

  @Getter private FilterType type;
    ...
    switch (type) {
      case Range:
        Preconditions.checkArgument(
            rejectValue == null, "For RangeFilter, rejectValue property is null");
        return new RangeFilter(fieldSchema, acceptValue);
      case Set:
        if (this.acceptValue != null) return new SetAcceptFilter(fieldSchema, acceptValue);
        if (this.rejectValue != null) return new SetRejectFilter(fieldSchema, rejectValue);
      default:
