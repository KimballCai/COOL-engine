...
public class DeltaCompressor implements Compressor {
  ...
  public int compress(int[] src, int srcOff, int srcLen, byte[] dest, int destOff, int maxDestLen) {
    ...
    Histogram hist = Histogram.builder().max(maxDelta).numOfValues(numOfVal).build();
    ...
    return HEADACC
        + compressor.compress(
            valuesToCompress, 0, srcLen, dest, destOff + offset, maxDestLen - offset);
  }
