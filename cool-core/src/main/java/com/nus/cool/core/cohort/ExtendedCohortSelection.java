...
public class ExtendedCohortSelection implements Operator {
      ...
      for (ExtendedFieldSet fs : selectors) {
        ...
        filterMap.put(
            ...
            FieldFilterFactory.create(
                tableSchema.getField(field), fs, fs.getFieldValue().getValues()));
      }
    }
  ...
  public void process(MetaChunkRS metaChunk) {
    ...
    for (Map<String, FieldFilter> birthFilter : birthFilters) {
      for (Map.Entry<String, FieldFilter> entry : birthFilter.entrySet()) {
        ...
        else entry.getValue().accept(metaField);
      }
      idx++;
    }
    ...
    this.maxDate =
        TimeUtils.getDate(
            metaChunk
                .getMetaField(tableSchema.getActionTimeFieldIdx(), FieldType.ActionTime)
                .getMaxValue());
  }
  ...
  public void process(ChunkRS chunk) {
    ...
    for (int i = 0; i < birthFilters.size(); i++) {
      ...
      for (Map.Entry<String, FieldFilter> entry : filterMap.entrySet()) {
        ...
        if (minTriggerTime[i] > 0) bUserActiveChunk &= entry.getValue().accept(field);
        else
  ...
  /**
   * @brief find the next tuple where the given @param event is qualified with respect to the birth
   *     filter
   * @param event
   * @param fromOffset
   * @param endOffset
  ...
  private int skipToNextQualifiedBirthTuple(int event, int fromOffset, int endOffset) {
    ...
    if (fromOffset >= endOffset || numFilter == 0) return fromOffset;
    ...
    while (true) {
      for (FieldFilter currentFilter : filterCollection) {
        ...
        if (passed == numFilter || fromOffset >= endOffset) return fromOffset;
      }
    }
  }

  /**
   * @brief check if the number of occurrence of each birth event follows the max/min trigger
   *     condition or not
  ...
  private boolean checkOccurrence(int e) {
    ...
    if (minTriggerTime[e] > occurrence
        || (maxTriggerTime[e] >= 0 && maxTriggerTime[e] < occurrence)) return false;
    return true;
  }

  /**
   * @brief check for each birth event if its aggregated value passes the aggregation filter or not
  ...
  private boolean filterByBirthAggregation() {
    ...
    for (int i = 0; r && i < birthAggFilters.size(); i++) {
      ...
      for (Map.Entry<String, FieldFilter> entry : filters.entrySet()) {
        ...
        if (birthAttr == null) return false; // aggregation value does not exist
        ...
        if (entry.getValue() instanceof SetFieldFilter) {
          ...
          if (value.getType() != ExtendedFieldSet.FieldValueType.AbsoluteValue) {
            filter.updateValues(
                getBirthAttribute(
                    value.getBaseEvent(), tableSchema.getFieldID(value.getBaseField())));
          }
  ...
  /**
   * @brief evaluate the birth filters against tuples within the given offset range
  ...
  private int getUserBirthTime(int start, int end) {
    ...
    for (Integer e : sortedEvents) {
      ...
      if (window == null) {
        ...
        int bday;
        if (minTriggerTime[e] == 0) bday = timeVector.get(offset);
        else bday = timeVector.get(offset - 1) + 1;
        ...
        while (startDay <= endDay) {
          ...
          while (!occurrences.isEmpty()) {
            if (occurrences.getFirst() >= windowOffset) break;
      ...
      for (int idx = 0; idx < events.size(); ++idx) {
        ...
        for (BirthSequence.CohortField cf : be.getCohortFields()) {
          ...
          if (fieldID == tableSchema.getActionTimeFieldIdx()) {
            cohort.addDimension(
                TimeUtils.getDateofNextTimeUnitN(
                    cohort.getBirthDate(), query.getAgeField().getUnit(), 0));
            continue;
          }
  ...
  private void filterAgeActivity(
      int ageOff, int ageEnd, BitSet bs, InputVector fieldIn, FieldFilter ageFilter) {
  ...
  public void selectAgeByActivities(int ageOff, int ageEnd, BitSet bs) {
    ...
    for (Map.Entry<String, FieldFilter> entry : ageByFilters.entrySet()) {
      this.filterAgeActivity(
          ageOff, ageEnd, bs, chunk.getField(entry.getKey()).getValueVector(), entry.getValue());
    }
  ...
  public String selectAgeActivities(int ageOff, int ageEnd, BitSet bs, BitSet ageDelimiters) {
    ...
    for (Map.Entry<String, FieldFilter> entry : ageFilters.entrySet()) {
      ...
      filterAgeActivity(
          ageOff, ageEnd, bs, chunk.getField(entry.getKey()).getValueVector(), ageFilter);
    }
