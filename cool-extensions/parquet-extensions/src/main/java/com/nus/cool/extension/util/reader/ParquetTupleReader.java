...
/**
 * This reader encapsulates the ParquetReader provided by Apache Parquet, and emit a record in a
 * Json String to be interpreted by JsonTupleParser. Currently, we do not expose ParquetReader
 * configuration.
 */
public class ParquetTupleReader implements TupleReader {
  private ParquetReader<Group> reader;
  private MessageType srcSchema;
  // private TableSchema requestedSchema; [TBD] we could add a requested schema to filter out
  // unnecessary column accesses.
  ...
  public boolean checkSchemaCompatibility(TableSchema requestedSchema) {
    return requestedSchema.getFields().stream()
        .allMatch(x -> this.srcSchema.containsField(x.getName()));
  }

  /**
   * check whether the reader finishes reading
   *
   * @return 1 denotes the reader still needs to read and 0 denoets there is nothing left to read.
   */
  @Override
